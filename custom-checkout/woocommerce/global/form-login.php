...
// Check if login is enabled in custom checkout settings.
if (class_exists('Weblo_Custom_Checkout')) {
	$settings = get_option('weblo_cc_settings', []);
	if (empty($settings['enable_custom_checkout']) || empty($settings['enable_login'])) {
		return;
	}
}

?>
<form class="woocommerce-form woocommerce-form-login login custom_login_form_wrapper" method="post"
	...
	<p class="form-row form-row-first">
		<label for="username"><?php esc_html_e('Username or email', 'woocommerce'); ?>&nbsp;<span class="required"
