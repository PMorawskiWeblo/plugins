...
<div class="wrap">
	...
	<form method="post" action="options.php">
		...
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><label for="wpp_max_upload_mb"><?php esc_html_e( 'Max upload size (MB)', 'woo-product-personalizer' ); ?></label></th>
				<td><input type="number" id="wpp_max_upload_mb" name="wpp_settings[max_upload_mb]" value="<?php echo esc_attr( $options['max_upload_mb'] ); ?>" min="1" max="100" class="small-text" /></td>
			</tr>
			<tr>
				...
				<td>
					<?php foreach ( UploadMimeTypes::definitions() as $mime => $definition ) : ?>
						...
						<label style="display:block;margin-bottom:4px;">
							<input type="checkbox" name="wpp_settings[allowed_mime_types][]" value="<?php echo esc_attr( $mime ); ?>" <?php checked( $checked ); ?> />
							<?php
							echo esc_html(
								sprintf(
									'%s (%s)',
			...
			<tr>
				<th scope="row"><label for="wpp_default_button_label"><?php esc_html_e( 'Default button label', 'woo-product-personalizer' ); ?></label></th>
				<td><input type="text" id="wpp_default_button_label" name="wpp_settings[default_button_label]" value="<?php echo esc_attr( $options['default_button_label'] ); ?>" class="regular-text" /></td>
			</tr>
			<tr>
				<th scope="row"><label for="wpp_default_accept_text"><?php esc_html_e( 'Default acceptance text', 'woo-product-personalizer' ); ?></label></th>
				<td><textarea id="wpp_default_accept_text" name="wpp_settings[default_accept_text]" rows="3" class="large-text"><?php echo esc_textarea( $options['default_accept_text'] ); ?></textarea></td>
			</tr>
		</table>
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<label><input type="radio" name="wpp_settings[frontend_mode]" value="modal" <?php checked( $options['frontend_mode'], 'modal' ); ?> /> <?php esc_html_e( 'Modal', 'woo-product-personalizer' ); ?></label><br />
					<label><input type="radio" name="wpp_settings[frontend_mode]" value="inline" <?php checked( $options['frontend_mode'], 'inline' ); ?> /> <?php esc_html_e( 'Inline', 'woo-product-personalizer' ); ?></label>
				</td>
			</tr>
			<tr>
				<th scope="row"><label for="wpp_button_position"><?php esc_html_e( 'Button position', 'woo-product-personalizer' ); ?></label></th>
				<td>
					<select id="wpp_button_position" name="wpp_settings[button_position]">
						...
						foreach ( $positions as $value => $label ) :
							?>
							<option value="<?php echo esc_attr( $value ); ?>" <?php selected( $options['button_position'], $value ); ?>><?php echo esc_html( $label ); ?></option>
						<?php endforeach; ?>
					</select>
					<p class="description"><?php esc_html_e( 'Shortcode: [woo_product_personalizer]', 'woo-product-personalizer' ); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Shortcode only', 'woo-product-personalizer' ); ?></th>
				<td><label><input type="checkbox" name="wpp_settings[shortcode_only]" value="1" <?php checked( $options['shortcode_only'] ); ?> /> <?php esc_html_e( 'Hide automatic button placement', 'woo-product-personalizer' ); ?></label></td>
			</tr>
		</table>
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<label><input type="checkbox" name="wpp_settings[cleanup_enabled]" value="1" <?php checked( $options['cleanup_enabled'] ); ?> /> <?php esc_html_e( 'Enable scheduled cleanup', 'woo-product-personalizer' ); ?></label>
					<p class="description">
			...
			<tr>
				<th scope="row"><label for="wpp_cleanup_interval"><?php esc_html_e( 'Cleanup interval', 'woo-product-personalizer' ); ?></label></th>
				<td>
					<select id="wpp_cleanup_interval" name="wpp_settings[cleanup_interval]">
						<option value="7" <?php selected( $options['cleanup_interval'], 7 ); ?>><?php esc_html_e( 'Every 7 days', 'woo-product-personalizer' ); ?></option>
						<option value="14" <?php selected( $options['cleanup_interval'], 14 ); ?>><?php esc_html_e( 'Every 14 days', 'woo-product-personalizer' ); ?></option>
						<option value="30" <?php selected( $options['cleanup_interval'], 30 ); ?>><?php esc_html_e( 'Every 30 days', 'woo-product-personalizer' ); ?></option>
					</select>
					<p class="description">
			...
			<tr>
				...
				<td>
					<label><input type="checkbox" name="wpp_settings[cleanup_only_completed]" value="1" <?php checked( $options['cleanup_only_completed'] ); ?> /> <?php esc_html_e( 'Only delete folders for completed/cancelled/refunded orders', 'woo-product-personalizer' ); ?></label>
					<p class="description">
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<label><input type="checkbox" name="wpp_settings[debug_enabled]" value="1" <?php checked( $options['debug_enabled'] ); ?> /> <?php esc_html_e( 'Enable debug mode', 'woo-product-personalizer' ); ?></label>
					<p class="description">
					...
					<ul class="description" style="list-style:disc;margin-left:18px;">
						<li><?php esc_html_e( 'A debug panel on the product page (state, layout, slots, recent events).', 'woo-product-personalizer' ); ?></li>
						<li><?php esc_html_e( 'Prefixed [WPP] messages in the browser console.', 'woo-product-personalizer' ); ?></li>
						<li><?php esc_html_e( 'Selected client-side events sent to the server log (AJAX).', 'woo-product-personalizer' ); ?></li>
						<li><?php esc_html_e( 'PHP-side debug/info/warning/error entries from the plugin when logging is active.', 'woo-product-personalizer' ); ?></li>
					</ul>
					<p class="description">
					...
					<p class="description">
						...
						<code style="display:block;margin-top:4px;word-break:break-all;"><?php echo esc_html( $wpp_paths['debug_log'] ); ?></code>
					</p>
					<p class="description">
	...
	<p class="description">
		...
		<code style="display:block;margin-top:4px;word-break:break-all;"><?php echo esc_html( $wpp_paths['orders_dir'] ); ?>*</code>
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
		...
		<p>
			<label for="wpp_manual_cleanup_days"><strong><?php esc_html_e( 'Delete folders', 'woo-product-personalizer' ); ?></strong></label><br />
			<select id="wpp_manual_cleanup_days" name="wpp_manual_cleanup_days">
				<?php foreach ( CleanupService::manual_interval_choices() as $days => $label ) : ?>
					<option value="<?php echo esc_attr( (string) $days ); ?>" <?php selected( $manual_cleanup_days, $days ); ?>><?php echo esc_html( $label ); ?></option>
				<?php endforeach; ?>
			</select>
		</p>
		<p class="description">
		...
		<p>
			<label><input type="checkbox" name="wpp_dry_run" value="1" /> <?php esc_html_e( 'Dry run (report only, do not delete)', 'woo-product-personalizer' ); ?></label>
		</p>
		<p class="description">
