...
(function ($) {
  $(document).ready(function () {
    ...
    // i18n strings
    let i18nStrCopyAll = __('Copy all to clipboard', 'acf-bober-code');
    let i18nStrCode = __('Code', 'acf-bober-code');
    ...
    $('#acftc-meta-box .inside').prepend(
      '<a href="#" class="acftc-copy-all acf-js-tooltip" title="' + i18nStrCopyAll + '"></a>'
    );
    ...
    fields.each(function (index) {
      ...
      if (data_type != 'tab' && data_type != 'message') {
        $(this)
          .find('.row-options')
          .eq(0)
          .append('<a class="acftc-scroll__link" href="#acftc-' + field_key + '">' + i18nStrCode + '</a>');
      }
    });
    ...
    $('a.acftc-scroll__link').click(function (event) {
      ...
      if (
        acfAdminToolbar.length &&
        (acfAdminToolbar.css('position') === 'fixed' || acfAdminToolbar.css('position') === 'sticky')
      ) {
      ...
      if (
        acfHeaderBar.length &&
        (acfHeaderBar.css('position') === 'fixed' || acfHeaderBar.css('position') === 'sticky')
      ) {
