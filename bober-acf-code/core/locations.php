...
class ACFTC_Locations
{
	...
	private static $locations_tc_pro = array(); // PRO locations treated as normal in this fork
...
?>
		<div class="acftc-location-settings">
			<div class="acftc-location-settings__inside">
				<div class="acf-field acf-field-select" data-name="style" data-type="select">
					<div class="acf-label">
						<label for="acf_field_group-style"><?php _e('Location', 'acf-bober-code'); ?></label>
						<?php echo $this->get_location_select_instructions_html(); ?>
					</div>
					<div class="acf-input">
						<select id="acftc-group-option" class="" data-ui="0" data-ajax="0" data-multiple="0" data-placeholder="<?php _e('Select', 'acf-bober-code'); ?>" data-allow_null="0">
							<?php foreach ($this->location_rules as $key => $location_rule) : ?>
								<option value="acftc-group-<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $this->get_location_clean_text($location_rule) ); ?></option>
							<?php endforeach; ?>
						</select>
					</div>
				</div>
	...
	public function get_location_select_instructions_html()
	{
		...
		if (version_compare(ACFTC_Core::$acf_version, '6', '<')) : ?>
			...
			<p class="description"><?php _e('Select a location to see the relevant theme code', 'acf-bober-code'); ?></p>

		<?php else : ?>

			<div class="acf-tip">
				<i tabindex="0" class="acf-icon acf-icon-help acf-js-tooltip" title="<?php _e('Select a location to see the relevant theme code', 'acf-bober-code'); ?>">?</i>
			</div>
	...
	private function get_locations_upgrade_notice_html($location_rule)
	{
		...
		$error_message_line_1 = sprintf(
			...
			__('The `%s` location is not available in this version of ACF Bober Code.', 'acf-bober-code'),
			$location_rule['param']
		);

		$error_message_line_2 = __('Alternatively add a Post or Page option to the Location Rules for this field group.', 'acf-bober-code');
	...
	?>
		<div class="acftc-field-meta">
			<span class="acftc-field-meta__title" data-pseudo-content="<?php echo esc_attr( $this->get_location_clean_text($location_rule) ); ?>"></span>
		</div>

		<div class="acftc-field-code">
			<a href="#" class="acftc-field__copy acf-js-tooltip" title="<?php _e('Copy to clipboard', 'acf-bober-code') ?>"></a>
			<pre class="line-numbers"><code class="language-php"><?php
																	echo htmlspecialchars("<?php // {$error_message_line_1} ?>\n");
																	echo htmlspecialchars("<?php // {$error_message_line_2} ?>\n");
																	if ( $error_message_line_3 ) {
