...
class ACFTC_Field
{
	...
	protected function get_render_partial()
	{
		...
		if ( empty( $this->type ) || ! preg_match( '/^[a-z0-9_\-]+$/i', $this->type ) ) {
			return '';
		}
	...
	private function is_debugging()
	{
		...
		return 'on' === sanitize_text_field( wp_unslash( $_GET['debug'] ) );
	}
	...
	private function get_field_html_title()
	{
		...
		if ( $this->is_debugging() ) {
			echo '<h3>Debug: ' . esc_html( $this->label ) . '</h3>';
		} else {
?>
			<span class="acftc-field-meta__title" data-type="<?php echo esc_attr( $this->type ); ?>" data-pseudo-content="<?php echo esc_attr( $this->label ); ?>"></span>
		<?php
		}
	...
	protected function get_field_html_body()
	{
		...
		if (file_exists($this->render_partial)) {
			...
			$error_message_1 = sprintf(
				...
				__('The `%s` field type is not supported in this plugin.', 'acf-bober-code'),
				$this->type
			);
		}
