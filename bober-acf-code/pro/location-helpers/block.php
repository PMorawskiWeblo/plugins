...
if (empty($block_location_name) || empty($block_info)) {
    ...
    $i18n_str_no_block_comment_title = __('No registered block selected.', 'acf-bober-code');
    $i18n_str_no_block_comment_1a = sprintf(
        /* translators: %s: > */
        __('1. Go to the Custom Fields %s Tools page and use the Register ACF Blocks or Options Page tool', 'acf-bober-code'),
        '>'
    );
    $i18n_str_no_block_comment_1b = __('to generate the code for registering a new block.', 'acf-bober-code');
    $i18n_str_no_block_comment_2 = __('2. Add the block registration code to your theme.', 'acf-bober-code');
    $i18n_str_no_block_comment_3a = __('3. Select your new block in the Locations meta box on this page and', 'acf-bober-code');
    $i18n_str_no_block_comment_3b = __('hit Update on this field group', 'acf-bober-code');
...
?>");
    ...
    // i18n strings
    $i18n_str_block_template_file = __('Block template file', 'acf-bober-code');
    $i18n_str_block_template = __('Block Template', 'acf-bober-code');
    $i18n_str_param_desc_block = __('The block settings and attributes.', 'acf-bober-code');
    $i18n_str_param_desc_content = __('The block inner HTML (empty).', 'acf-bober-code');
    $i18n_str_param_desc_is_preview = __('True during AJAX preview.', 'acf-bober-code');
    $i18n_str_param_desc_post_id = __('The post ID this block is saved to.', 'acf-bober-code');
    $i18n_str_comment_id_attr = sprintf(
        /* translators: %s: "anchor" */
        __('Create id attribute allowing for custom %s value.', 'acf-bober-code'),
        '"anchor"'
    );
    $i18n_str_comment_class_attr = sprintf(
        /* translators: 1: "className" 2: "align" */
        __('Create class attribute allowing for custom %1$s and %2$s values.', 'acf-bober-code'),
        '"className"',
        '"align"'
    );
    $i18n_str_comment_add_styles = __('Add styles that use ACF values here', 'acf-bober-code');
