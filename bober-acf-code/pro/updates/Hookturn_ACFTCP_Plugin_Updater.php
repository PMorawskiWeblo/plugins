...
class Hookturn_ACFTCP_Plugin_Updater
{
		...
		if (! empty($update_cache->response[$this->name]) && version_compare($this->version, $version_info->new_version, '<')) {
			...
			if (empty($version_info->download_link)) {
				printf(
					...
					__('There is a new version of %1$s available. %2$sView version %3$s details%4$s.', 'acf-bober-code'),
				...
				printf(
					...
					__('There is a new version of %1$s available. %2$sView version %3$s details%4$s or %5$supdate now%6$s.', 'acf-bober-code'),
					esc_html($version_info->name),
					'<a target="_blank" class="thickbox" href="' . esc_url($changelog_link) . '">',
					esc_html($version_info->new_version),
					'</a>',
	...
	public function show_changelog()
	{
		...
		if (! current_user_can('update_plugins')) {
			wp_die(__('You do not have permission to install plugin updates', 'acf-bober-code'), __('Error', 'acf-bober-code'), array('response' => 403));
		}
