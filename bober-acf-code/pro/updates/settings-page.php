...
function hookturn_acftcp_license_menu()
{
	...
	add_options_page(
		__('Theme Code Pro License', 'acf-bober-code'),
		__('Theme Code Pro', 'acf-bober-code'),
		'manage_options',
...
?>
	<div class="wrap acf-settings-wrap acftcp-license-page__wrap">
		<h2><?php _e('ACF Theme Code Pro', 'acf-bober-code'); ?></h2>
		...
		<div class="acf-box">
			...
			<div class="inner">
				...
				<p><?php _e('To unlock updates please enter your license key, save it and then activate it.', 'acf-bober-code'); ?></p>
				...
				<form method="post" action="options.php">
					...
					<table class="form-table">
						<tbody>
							<tr>
								<th>
									<label class="acftcp-license-page__label" for="hookturn_acftcp_license_key"><?php _e('License Key', 'acf-bober-code'); ?></label>
								</th>
								<td>
									...
									<?php if ($license == '') : ?>
										<p class="description"><?php _e('Enter your license key.', 'acf-bober-code'); ?></p>
									<?php endif; ?>
									<?php submit_button(
										__('Save License Key', 'acf-bober-code'),
							...
							<?php if ('' != $license) : ?>
								<tr>
									<th>
										<span class="acftcp-license-page__label"><?php _e('License Status', 'acf-bober-code'); ?></span>
									</th>
									<td>
										<?php if ($status !== false && $status == 'valid') : ?>
											<p class="acftcp-license-page__status acftcp-license-page__status--active"><?php _e('Active', 'acf-bober-code'); ?></p>
											<?php wp_nonce_field('hookturn_acftcp_nonce', 'hookturn_acftcp_nonce'); ?>
											<p class="submit">
												<input type="submit" class="button button-primary" name="edd_license_deactivate" value="<?php _e('Deactivate License', 'acf-bober-code'); ?>" />
											</p>
										<?php else : ?>
											<?php wp_nonce_field('hookturn_acftcp_nonce', 'hookturn_acftcp_nonce'); ?>
											<p class="acftcp-license-page__status acftcp-license-page__status--inactive"><?php _e('Inactive', 'acf-bober-code'); ?></p>
											<p class="description"><?php _e('Activate license to enable plugin updates.', 'acf-bober-code'); ?></p>
											<p class="submit">
												<input type="submit" class="button button-primary" name="edd_license_activate" value="<?php _e('Activate License', 'acf-bober-code'); ?>" />
											</p>
										<?php endif; ?>
									</td>
								</tr>
	...
	if (isset($_POST['edd_license_activate'])) {
		...
		if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
			...
			if (is_wp_error($response)) {
				...
				$message = __('An error occurred, please try again.', 'acf-bober-code');
			}
		} else {
				...
				switch ($license_data->error) {
					...
					case 'expired':
						...
						$message = sprintf(
							...
							__('<b>Your license key expired on %1$s</b>.<br/><br/>To continue receiving new features, security updates and support, log into your <a href="%2$s" target="_blank">Hookturn account</a> and renew your license.', 'acf-bober-code'),
					...
					case 'revoked':
						...
						$message = __('Your license key has been disabled.', 'acf-bober-code');
						break;
					...
					case 'missing':
						...
						$message = sprintf(
							...
							__('<b>Invalid license</b>.<br/><br/>Try entering your <em>License Key</em> again. Then hit the <em>Save License Key</em> button followed by the <em>Activate License</em> button. If the issue persists, log into your <a href="%s" target="_blank">Hookturn account</a> and check the status of your license.', 'acf-bober-code'),
							'https://hookturn.io/account/'
						);
						break;
					...
					case 'site_inactive':
						...
						$message = __('Your license is not active for this URL.', 'acf-bober-code');
						break;
					...
					case 'item_name_mismatch':
						...
						$message = sprintf(
							...
							__('This appears to be an invalid license key for %s.', 'acf-bober-code'),
							HOOKTURN_ITEM_NAME
						);
						break;
					...
					case 'no_activations_left':
						...
						$message = __('Your license key has reached its activation limit.', 'acf-bober-code');
						break;
		...
		if (! empty($message)) {
			...
			$notice_type = 'success';
			$message = __('<b>Licence key activated</b>.<br/><br/>Plugin updates are now enabled.', 'acf-bober-code');
		}
...
function hookturn_acftcp_deactivate_license()
{
	...
	if (isset($_POST['edd_license_deactivate'])) {
		...
		if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
			...
			$notice_type = 'warning';
			$message = __('<b>License key deactived</b>. Updates now disabled.', 'acf-bober-code');
		}
