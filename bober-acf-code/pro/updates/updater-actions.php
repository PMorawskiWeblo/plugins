...
function hookturn_acftcp_plugin_updater()
{
	...
	$edd_updater = new Hookturn_ACFTCP_Plugin_Updater(
		HOOKTURN_STORE_URL,
		ACFTC_PLUGIN_FILE,
		array(
...
function hookturn_modify_plugin_update_message($plugin_data, $response)
{
	...
	if (empty($response->package)) {
		...
		echo sprintf(
			...
			__('To enable updates, please save and activate your license key on the <a href="%1$s">Theme Code Pro Settings</a> page. If you don\'t have a licence key, please visit <a href="%2$s" target="_blank">Hookturn</a> for details and pricing.', 'acf-bober-code'),
			admin_url('options-general.php?page=theme-code-pro-license'),
			'https://hookturn.io/downloads/acf-bober-code-pro/'
		);
	}
}
		...
		foreach ($hook_extra['plugins'] as $plugin) {
			...
			if ($plugin == ACFTC_PLUGIN_BASENAME) {
				...
				$custom_error = sprintf(
					...
					__('please check the status of your license key on the <a href="%s">Theme Code Pro Settings</a> page.', 'acf-bober-code'),
					admin_url('options-general.php?page=theme-code-pro-license')
				);
