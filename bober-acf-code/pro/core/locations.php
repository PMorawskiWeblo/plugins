...
?>
	...
	?>
		<div class="acftc-field-meta">
			<span class="acftc-field-meta__title" data-pseudo-content="<?php echo esc_attr( $location_helper_title ); ?>"></span>
		</div>
...
<?php
	...
	private function get_location_helper_title($location_rule)
	{
		...
		switch ($location_slug) {
			...
			case 'current_user':
				return __('User Variables', 'acf-bober-code');
				break;
			...
			case 'attachment':
				return __('Attachment Variables', 'acf-bober-code');
				break;

			case 'taxonomy':
				return __('Taxonomy Term Variables', 'acf-bober-code');
				break;

			case 'comment':
				return __('Comment Variables', 'acf-bober-code');
				break;

			case 'widget':
				return __('Widget Variables', 'acf-bober-code');
				break;

			case 'block':
				return __('Block Template', 'acf-bober-code');
				break;
	...
	private function get_location_helper_php($location_rule)
	{
		...
		// Allow only safe filename chars to prevent path traversal
		if ( empty( $location_slug ) || ! preg_match( '/^[a-z0-9_\-]+$/i', $location_slug ) ) {
			return ob_get_clean();
		}
